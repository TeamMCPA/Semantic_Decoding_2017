...
DecodingResults.SemanticGroup = NaN;
...
chan_stable_cut = 50; % value of 1 effectively turns off stability
...
for subjnum = 1:length(Probe1List),
    ...
    subj_struct(subjnum).Condition = extract_vecs_1subj({p1_file_path, p2_file_path}, conditions, scan_window, 'obsv');
...
% decoding.
load('model_and_rsa_data/COMPOSES_semantic_vectors_01June2016.mat','semantic_matrix');
...
%% Repeat semantic decoding on the group-level model
% Average the RSA matrices for all subjects into a single group model, and
% apply the semantic matrix to decode this model.

DecodingResults.SemanticGroup = nanmean(pairwise_rsa_test(mean(sim_struct,3),semantic_sim_struct));

%% Plot semantic decoding
% Plots each subject's mean pairwise decoding accuracy against the
% semantic model (COMPOSES)
figure
bar([DecodingResults.Semantic DecodingResults.SemanticGroup])
title(sprintf('Semantic decoding accuracy, Mean: %0.2f, Group: %0.2f',...
    nanmean(DecodingResults.Semantic),...
    DecodingResults.SemanticGroup));
ax = gca;
xlabel('Subject ID');
ax.XTickLabel = [num2cell(1:length(subj_struct)),'group'];
fprintf('Semantic Mean: %0.2f\n',mean(DecodingResults.Semantic))
fprintf('Semantic Group: %0.2f\n',DecodingResults.SemanticGroup)
...
group_comp_model = pairwise_rsa_null(mean(sim_struct,3),semantic_sim_struct,10000);
p_comp = mean(mean(group_comp_model,2)>=DecodingResults.SemanticGroup);
disp(['Group COMPOSES Decoding: ' num2str(mean(DecodingResults.SemanticGroup)) ' p=' num2str(p_comp)]);
